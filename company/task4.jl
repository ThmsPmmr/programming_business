...
include("c:\\programming_business\\read_instance.jl")
...
function DijsktraAlgorithm(SPP::ShortestPathProblem)
    ...
    shortestPath = Vector{Int64}(undef,0)
    node = SPP.destinationNode
    #loop through the nodes and push them into the vector until we reach the start node
    while node != -1

        push!(shortestPath, node)
        node = L[node].origin
    ...
    return L[SPP.destinationNode].cost, shortestPath
...
# - shortestPaths: Vector of Vectors with all shortest Path between each delivery location pair
# #################################################################################
function computeShortestPath(G::Graph, Q::Vector, m::Int64)
    #initialize a matrix with only zeros
    d = zeros(m,m)
    shortestPaths= Vector{Vector{Int64}}(undef, 0)
    ...
    for i in 1:length(Q) 
        for j in 1:length(Q)
            if i != j
                ...
                d[i,j], path = DijsktraAlgorithm(St)
...
function connectivityCutsAlgorithm(model::Model, n::Int64, startLocation::Int64, Q::Vector, shortestPaths::Vector)
    ...
    #iterate through the delivery locations
    for deliveryLoc in sub[end][2:end]
        #iterate through all shortest Paths
        for path in shortestPaths

            #if the end of the path is equal to the next delivery location and the beginning of the path
            #is equal to the end of the current tour then add this specific path to the deliveryTour
            if path[end] == Q[deliveryLoc] && path[1] == deliveryTour[end]
                for node in path[2:end]
                    push!(deliveryTour, node)
...
connectivityCutsAlgorithm(tsp, m, startLocation, Q, shortestPaths)